...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights="yolov5s.pt", weights_only=True, device=torch.device("cpu"), dnn=False, data=None, fp16=False, fuse=True):
        ...
        if pt:  # PyTorch
            model = attempt_load(weights if isinstance(weights, list) else w, weights_only=weights_only, device=device, inplace=True, fuse=fuse)
